...
let _ = 
    ...
    | _ -> match Sys.argv.(1) with
           ...
           | "-h" | "--help" | "help" -> KwhaleCommand.kwhale_help ()
           | "init" -> Printf.printf "init"
           | "new" -> Printf.printf "new"
           | "run" -> (match Array.length Sys.argv with
                      | 2 -> Printf.printf "\027[1m\027[31mError\027\027[0m\027[1m: file doesn\'t specified\027\n"
                      | _ -> KwhaleCommand.kwhale_run (Sys.argv.(2)) ())
           | "to" -> Printf.printf "to"
           | "-v" | "--version" | "version" -> KwhaleCommand.kwhale_version ()
